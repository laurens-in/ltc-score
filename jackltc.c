...
// represent event in score in human readable timecodes
struct note {
  char start[6];
  char end[6];
  char* description;
...
// parse string timestamps to int representing nr. of seconds
int parseTime(char * timestamp) {
  char* segment = strtok(timestamp, ":");
  int minutes = atoi(segment);
  segment = strtok(NULL, ":");
  int seconds = atoi(segment);
...
int main (int argc, char **argv) {
  ...
  // enter notes here
  struct note parts[2] = {
    {"0:00", "00:30", "first part"},
    {"00:30", "08:00", "second part"}
  };

  // parse them into events 
